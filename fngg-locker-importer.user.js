...
(function() {
    ...
    // sorting - lower number = shows first
    var typeOrder = {
        ...
        toy: 15, aura: 16,
        guitar: 17, bass: 17, drum: 17, keytar: 17, microphone: 17, // festival stuff
        car: 18, decal: 18, wheel: 18, trail: 18, boost: 18, // racing
    ...
    // maps weird api names to our names
    var typeMap = {
    ...
    // backup - guess type from item id if api doesnt tell us
    var idPatterns = [
    ...
    // series skins sort before regular ones
    var seriesBonus = {
    ...
    function loadSession() {
        try {
            ...
            if (!d || Date.now() - d.ts > 7200000) return null; // 2h
        ...
        for (var i = 0; i < eps.length; i++) {
            try {
                ...
                for (var j = 0; j < r.data.data.length; j++) {
                    ...
                    // some items have series name as rarity (marvel etc)
            ...
            } catch(e) { /* some endpoints fail, whatever */ }
        }
    ...
    async function doImport() {
        ...
        try {
            ...
            if (fr.status !== 200) throw 'fngg fail';
            var fngg = {};
            ...
            if (ar.status === 401) { clearSession(); updateUI(); toast('Session expired', 'err'); working = false; return; }
